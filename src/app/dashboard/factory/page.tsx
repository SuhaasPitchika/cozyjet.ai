...
export default function ContentFactoryPage() {
  ...
  const handleRunCompliance = async () => {
    setIsVerifying(true);
    try {
      const report = await snooksComplianceCheck({
        content: context,
        platform: selectedPlatform || "general",
        exclusionList: ["Project X", "Client Alpha", "Confidential Revenue"]
      });
      ...
      toast({
        ...
        description: "The VERA immune system encountered an error."
      });
    } finally {
      ...
      <AnimatePresence mode="wait">
        {currentStep === 1 && (
          <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {PLATFORMS.map((p) => (
              <div key={p.id} onClick={() => { setSelectedPlatform(p.id); setCurrentStep(2); }} className="p-8 bg-zinc-900/50 border border-white/5 rounded-2xl cursor-pointer group hover:border-amber-500 transition-all hover:bg-zinc-900 hover:scale-[1.02]">
          ...
          <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['Case Study', 'Thought Leadership', 'Micro-Tutorial'].map((t) => (
                <div key={t} onClick={() => setCurrentStep(3)} className="p-6 bg-zinc-900/50 border border-white/5 rounded-xl cursor-pointer hover:border-amber-500 hover:bg-zinc-900 transition-all text-center">
        ...
        {currentStep === 3 && (
          <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
            ...
            <div className="flex flex-col items-center gap-6 pt-10">
              ...
              >
                {isVerifying ? <Loader2 className="animate-spin" /> : <>RUN COMPLIANCE CHECK <ShieldCheck className="w-5 h-5 ml-3" /></>}
              </Button>
            </div>
          </motion.div>
        )}

        {currentStep === 4 && complianceReport && (
          <motion.div key="step4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(complianceReport).map(([key, value]: [string, any]) => (
                key !== 'overallStatus' && (
                  <div key={key} className="p-4 rounded-xl bg-zinc-900/50 border border-white/5 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">{key.replace('Check', '')}</p>
                      <p className="text-xs text-white">{value.message}</p>
                    </div>
                    <Badge className={cn(
                      value.status === 'passed' ? "bg-green-500/20 text-green-500" :
                      value.status === 'warning' ? "bg-amber-500/20 text-amber-500" :
            ...
            <div className="flex justify-center pt-10">
              <Button className="bg-black text-white px-10 h-14 rounded-full font-bold text-xs uppercase tracking-[0.2em]">
                INITIALIZE GENERATION <Send size={14} className="ml-3" />
              </Button>
            </div>
          </motion.div>
        )}
